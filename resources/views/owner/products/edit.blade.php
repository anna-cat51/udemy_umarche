                  ...
                  <form method="post" action="{{ route('owner.products.update', ['product' => $product->id])}}">
                      ...
                      <div class="p-2 w-1/2 mx-auto">
                        <div class="relative flex justify-around">
                            <div><input type="radio" name='type' value="{{ \Constant::PRODUCT_LIST['add'] }}" class="ml-2" checked>追加</div>
                            <div><input type="radio" name='type' value="{{ \Constant::PRODUCT_LIST['reduce'] }}" class="ml-2" >削減</div>
                        </div>
                      </div>
                      <div class="p-2 w-1/2 mx-auto">
                        <div class="relative">
                      ...
                      <x-select-image :images="$images" carrentId="{{ $product->image1 }}" currentImage="{{ $product->imageFirst->filename ?? '' }}" name="image1" />
                      <x-select-image :images="$images" carrentId="{{ $product->image2 }}" currentImage="{{ $product->imageSecond->filename ?? '' }}" name="image2" />
                      <x-select-image :images="$images" carrentId="{{ $product->image3 }}" currentImage="{{ $product->imageThird->filename ?? '' }}" name="image3" />
                      <x-select-image :images="$images" carrentId="{{ $product->image4 }}" currentImage="{{ $product->imageFourth->filename ?? '' }}" name="image4" />
                      <x-select-image :images="$images" name="image5" />
                      <div class="p-2 w-1/2 mx-auto">
                        <div class="relative flex justify-around">
                            <div><input type="radio" name='is_selling' value="1" class="ml-2" @if($product->is_selling ===1){ checked } @endif>販売中</div>
                            <div><input type="radio" name='is_selling' value="0" class="ml-2" @if($product->is_selling ===0){ checked } @endif>停止中</div>
                        </div>
                      </div>
